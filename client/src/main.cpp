...
int main() {
    ...
    sf::Sprite backgroundSprite(backgroundTex);
    {
        ...
        float scale = worldHeight / static_cast<float>(bgSize.y);
    ...
    while (window.isOpen()) {
        ...
        while (auto evtOpt = window.pollEvent()) {
            ...
            if (evt.is<sf::Event::Resized>()) {
                ...
                float newAspect = static_cast<float>(r->size.x) / static_cast<float>(r->size.y);
                    ...
                    for (auto &packet : *pkts) {
                        switch (packet.header.type) {
                            ...
                            case PacketType::kPlayerReadyCount: {
                                if (auto prc = network::PacketFactory<PacketType>
                            ...
                            case PacketType::kPong: {
                                uint32_t sent;
                                std::memcpy(&sent, packet.body.data(), sizeof(sent));
                            ...
                            case PacketType::kPlayerScore: {
                                if (auto psp = network::PacketFactory<PacketType>
                                    ...
                                    if (localPlayerId && psp->player_id == *localPlayerId) {
                                        ...
                                        std::string prefix = hasWon
                                                            ? "You Win! Score: "
                                                            : "You Lose! Score: ";
                                        scoreText.setString(
                                            prefix + std::to_string(finalScore)
                                        );
                                        scoreText.setFillColor(
                                            hasWon ? sf::Color::Green : sf::Color::Red
                                        );
                                    }
                                }
                                break;
